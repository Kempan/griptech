...
export const getCategoryTopLevelBySlug = async (
	req: Request,
	res: Response
): Promise<void> => {
	try {
		...
		while (topCategory.parentId) {
			...
			if (!parentCategory) break; // If parent is not found, stop

			topCategory = parentCategory;
		}

		// Fetch the top category again, now including its children
		const fullCategoryData = await prisma.productCategory.findUnique({
